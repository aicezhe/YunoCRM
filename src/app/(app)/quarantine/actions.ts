...
export async function createCompanyAndResolve(
  ...
  if (!item || item.status !== "open") return { ok: false, error: "This item is no longer open." };
  ...
  try {
    ...
    if (message.includes("companies_domain_key")) {
      return { ok: false, error: "A company with that domain already exists — use Link to existing instead." };
    }
...
export async function linkToExistingAndResolve(quarantineItemId: string, companyId: string): Promise<ActionResult> {
  ...
  try {
    ...
    return { ok: false, error: "Couldn't link this item. Try again." };
  }
}
...
export async function discardQuarantineItem(quarantineItemId: string): Promise<ActionResult> {
  ...
  try {
    ...
    return { ok: false, error: "Couldn't discard this item. Try again." };
  }
}
